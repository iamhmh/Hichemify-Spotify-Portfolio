...
function FullScreenPlayer({ toggle, state, controls, volumeIcon }) {
	...
	return (
		<div className="h-full relative" onClick={controls[state?.playing ? 'pause' : 'play']}>
			<div className="absolute inset-0 object-cover bg-center bg-cover blur-md opacity-30" style={{backgroundImage: `url(${current.image2})`}} />
			...
			<div className="absolute opacity-70 top-8 left-8 gap-x-4 text-white flex items-center">
				...
				<div className="text-xs">
					...
					<h6 className="font-semibold mt-0.5">{current.titleSpotify}</h6>
				</div>
			</div>

			<div className="absolute left-8 bottom-36 flex items-center gap-x-5">
				<img src={current.image2} alt="" className="w-24 h-24 object-cover"/>
				<div className="self-end">
					<h3 className="text-3xl font-bold">{current.titleSpotify}</h3>
